function decoderout = decoderfn_unquant(decoderin,rateid,numBits,NoiseVariance,prm80211p)
...
UnquantizedValue = demodSigBinary;
...
% UnquantizedValue = double(UnquantizedValue); %Nakagami NaN decoding failure
% UnquantizedValue( isnan(UnquantizedValue) )=0; %Lines
% UnquantizedValue = changem(UnquantizedValue,[10 -10],[Inf -Inf]);

if (rateid == 1 ||  rateid == 3 ||rateid == 5)
    demodout= vitdec(UnquantizedValue,prm80211p.trellis,tbl,'term','unquant');
elseif (rateid == 2 ||rateid == 4 || rateid == 6 ||rateid == 8)
    %codepuncturepattern=[1 1 0 1 1 0].'; for rate 3/4
    codepuncturepattern34=[1 1 0 1 1 0].';
    demodout= vitdec(UnquantizedValue,prm80211p.trellis,tbl,'term','unquant',codepuncturepattern34);
elseif (rateid == 7)
    %codepuncturepattern=[1 1 1 0].'; for rate 2/3
    codepuncturepattern23=[1 1 1 0].';
    demodout= vitdec(UnquantizedValue,prm80211p.trellis,tbl,'term','unquant',codepuncturepattern23);
end
